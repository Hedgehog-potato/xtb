...
!    Dummy
     ...
     if (.not.allocated(topo%dqadchi)) allocate( topo%dqadchi(86,n), source = 0.0d0 )
     if (.not.allocated(topo%dqadgam)) allocate( topo%dqadgam(86,n), source = 0.0d0 )
     if (.not.allocated(topo%dqadcnf)) allocate( topo%dqadcnf(86,n), source = 0.0d0 )
     if (.not.allocated(topo%dqadalp)) allocate( topo%dqadalp(86,n), source = 0.0d0 )
     if (.not.allocated(topo%der_f)) allocate( topo%der_f(3,n), source = 0.0d0 )
     ...
     if (.not.allocated(topo%vangl)) allocate( topo%vangl(4,topo%nangl_alloc), source = 0.0d0 )
     if (.not.allocated(topo%vtors)) allocate( topo%vtors(3,topo%ntors_alloc), source = 0.0d0 )
...
!    Dummy
     ...
     if (allocated(topo%dqadchi)) deallocate( topo%dqadchi)
     if (allocated(topo%dqadgam)) deallocate( topo%dqadgam)
     if (allocated(topo%dqadcnf)) deallocate( topo%dqadcnf)
     if (allocated(topo%dqadalp)) deallocate( topo%dqadalp)
     if (allocated(topo%der_f)) deallocate( topo%der_f)
